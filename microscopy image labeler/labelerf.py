...
for imagenamme in imena:
          ...
          #  if kk>6000:
           #     break
            for j in range(0,gray.shape[1]-Nj-jstep,int(jstep)):
                ...
                kooor[kk,:]=((i+iil/2-bordersize),(j+iil/2-bordersize),(iil/4.0))  #-bordersize -bordersize
                   # print(kooor[kk,:])
                kk=kk+1
    ...
    while iil<430:
        ...
        Ni,Nj=(iil,iil)
        print(iil/1.8)
        istep=max(2,int(iil/15))
        jstep=max(2,int(iil/15))
          ...
          #  if kk>6000:
           #     break
            for j in range(0,gray.shape[1]-Nj-jstep,int(jstep)):
                ...
                img[kk,:]=np.reshape(cv2.resize(gray[i:i+Ni,j:j+Nj],dsize=(24,24),interpolation=inter),576)
                kooor[kk,:]=((i+iil/2-bordersize),(j+iil/2-bordersize),(iil/1.8))  #-bordersize -bordersize
                   # print(kooor[kk,:])
                kk=kk+1
